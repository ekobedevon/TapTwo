				...
				<div class="hidden md:ml-6 md:flex md:items-center md:space-x-4">
					...
					{#each options as option}
						...
						{:else}
							<a
								href={option.path}
								class="rounded-md px-3 py-2 text-sm font-medium text-text hover:bg-primary/5"
								>{option.title}</a
							>
						{/if}
					{/each}
					...
					<div class="hidden md:ml-4 md:flex md:shrink-0 md:items-center">
						...
						<div class="relative ml-3">
							...
							>
								...
								<User classData="size-8 fill-primary border-2 rounded-full border-primary" />
							</button>

							{#if show_profile}
								<div
									id="profileDropdown"
									class="absolute right-0 w-48 rounded-md bg-white py-1 shadow-lg outline-1 outline-black/5 transition transition-discrete [--anchor-gap:--spacing(2)] data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-200 data-enter:ease-out data-leave:duration-75 data-leave:ease-in"
								>
									<a
										href="/"
			...
			<div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
				...
				{#each options as option}
					...
					{:else}
						<a
							href={option.path}
							class="block rounded-md px-3 py-2 text-base font-medium text-text hover:bg-accent hover:text-text"
							>{option.title}</a
						>
					{/if}
				{/each}
